...
func TestApplyUserFilter_EmptyAddsNoWhere(t *testing.T) {
	for _, name := range []string{"", "   ", "\t\n"} {
		...
		query, args, err := repositories.ApplyUserFilter(builder, repositories.UserFilter{Name: name}).ToSql()
		if err != nil {
			t.Fatalf("ToSql: %v", err)
		}
...
func TestApplyUserFilter_BindsValueAsArgument(t *testing.T) {
	...
	query, args, err := repositories.ApplyUserFilter(builder, repositories.UserFilter{Name: evil}).ToSql()
	if err != nil {
		t.Fatalf("ToSql: %v", err)
	}
...
func TestApplyUserFilter_TrimsValue(t *testing.T) {
	...
	_, args, err := repositories.ApplyUserFilter(builder, repositories.UserFilter{Name: "  ada  "}).ToSql()
	if err != nil {
		t.Fatalf("ToSql: %v", err)
	}
...
func TestListQuery_FilterIsBoundArgument(t *testing.T) {
	query, args, err := repositories.BuildListUsersQuery(repositories.UserFilter{Name: "ada"}, 10, 0)
	if err != nil {
		t.Fatalf("BuildListUsersQuery: %v", err)
	}
