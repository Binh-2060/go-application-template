...
*/
func TestHTTP_MissingUserIs404(t *testing.T) {
	app := newTestApp()

	status, _ := do(t, app, http.MethodGet, "/users/00000000-0000-0000-0000-000000000000", nil)

	if status != http.StatusNotFound {
		t.Fatalf("GET missing user = %d, want 404", status)
	}
}
...
/*
PATCH with no recognised field cannot be turned into an UPDATE, and is the
client's mistake: 400, not 500.
*/
...
/*
The partial-update guarantee, seen from outside: send one field, the other keeps
its stored value.
*/
func TestHTTP_PatchIsPartial(t *testing.T) {
	app := newTestApp()
	m := marker(t)
	...
	status, env := do(t, app, http.MethodPatch, "/users/"+id, map[string]string{"name": m + "Grace"})
	if status != http.StatusOK {
	...
	if updated.Surename != "Lovelace" {
		t.Errorf("Surename = %q, want it unchanged", updated.Surename)
	}
}
...
func TestHTTP_ListFilterAndPagination(t *testing.T) {
	...
	status, env := do(t, app, http.MethodGet, "/users?name="+m+"&page=1&per_page=2", nil)
	if status != http.StatusOK {
...
func TestHTTP_ListWithNoMatchesIsEmptyArray(t *testing.T) {
	...
	_, env := do(t, app, http.MethodGet, "/users?name="+marker(t), nil)
...
func TestHTTP_Delete(t *testing.T) {
	...
	if status, _ := do(t, app, http.MethodGet, "/users/"+id, nil); status != http.StatusNotFound {
		t.Fatalf("GET after delete = %d, want 404", status)
	}

	// Deleting it again is now the missing-row case.
	if status, _ := do(t, app, http.MethodDelete, "/users/"+id, nil); status != http.StatusNotFound {
