...
func TestListUsers_AppliesDefaultsForZeroValues(t *testing.T) {
	...
	page, err := services.ListUsers(context.Background(), requestbody.ListUsers{Name: marker(t)})
	if err != nil {
...
func TestListUsers_TotalPageRoundsUp(t *testing.T) {
	...
	page, err := services.ListUsers(ctx, requestbody.ListUsers{Name: m, Page: 1, PerPage: 2})
	if err != nil {
		t.Fatalf("services.ListUsers: %v", err)
	}
