...
- **`pkg/jwt`** — sign/verify JWTs, not wired into any route yet (no auth feature exists). Two independent managers, both returning `*jwt.Claims` (`jwt.RegisteredClaims`) from `Verify`: `Manager` (HS256, `NewManager(ConfigFromEnv())`, needs `JWT_SECRET`) and `RSAManager` (RS256, `NewRSAManager(RSAConfigFromEnv())`, needs `JWT_RSA_PUBLIC_KEY_PATH` and, unless it's a verify-only instance, `JWT_RSA_PRIVATE_KEY_PATH` — PEM files, not inline env vars). `NewRSAManager` rejects keys under 2048 bits; there is no 1024-bit option, RSA-1024 is broken by modern standards. `Verify` on both pins the expected algorithm family (HMAC vs RSA) to avoid `alg` confusion attacks, and checks `Issuer`. `Sign`/`SignWithTTL` are symmetric across both types, so a feature can swap Manager for RSAManager without touching call sites.
